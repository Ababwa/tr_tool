...
A field must be either `flat` or `delegate`.

A field may not be both `boxed` and `list`.
*/
#[proc_macro_derive(Readable, attributes(flat, delegate, boxed, list))]
...
// 			(Some(named), InitializerType::Named)
// 		},
// 		Data::Struct(DataStruct { fields: Fields::Unnamed(FieldsUnnamed { unnamed, .. }), .. }) => {
// 			(Some(unnamed), InitializerType::Tuple)
// 		},
...
// 				None => Cow::Owned(
// 					Ident::new(&format!("field{}", tuple_field_num.next().unwrap()), Span::call_site()),
// 				),
// 			};
