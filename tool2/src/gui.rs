use std::{
	time::Duration, num::NonZeroU32, future::Future, thread::{sleep, spawn},
	sync::{Arc, mpsc::{channel, TryRecvError}},
};
...
use winit::{
	dpi::{PhysicalPosition, PhysicalSize}, event::{DeviceEvent, ElementState, Event, KeyEvent, MouseButton, MouseScrollDelta, WindowEvent}, event_loop::{EventLoop, EventLoopWindowTarget}, keyboard::{KeyCode, ModifiersState, PhysicalKey}, platform::windows::WindowBuilderExtWindows, window::{Icon, Window, WindowBuilder}
};
...
pub fn run<T: Into<String>, G: Gui, F: FnOnce(&Device, UVec2) -> G>(
	...
	let mut gui = make_gui(&device, window_size.to_vec());
	...
	let result = event_loop.run(|event, target| match event {
		Event::DeviceEvent { event: DeviceEvent::MouseMotion { delta: (x, y) }, .. } => {
			gui.mouse_moved(window_size.to_vec(), &queue, dvec2(x, y));
		},
		Event::WindowEvent { event, .. } => if !egui_input_state.on_window_event(&window, &event).consumed {
			match event {
				...
				WindowEvent::Resized(new_size) => {
					...
					gui.resize(window_size.to_vec(), &device, &queue);
				},
